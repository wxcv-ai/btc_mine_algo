...
class Point:
    ...
    def __add__(self, other: Point) -> Point:
        ...
        print("end"  ,'||||',len(str( (m*(rx - self.x) + self.y)  )),"||||",len(str(m)) )
        print(value)
        print("\n")
        Point.all_data_test.append({"value":value,"rx" :rx , "ry" :ry , "m":m , "self_x":self.x ,
         "self_y":self.y ,"other_x":other.x ,"other_y":other.y 
         ,"rx_before_mod" :(m**2 - self.x - other.x)
         ,"ry_before_mod":(-(m*(rx - self.x) + self.y)),"p" :self.curve.p})
    ...
    def __rmul__(self, k: int) -> Point:
        ...
        result = INF
        append = self
        #print(len(str(k))*8 , type(result) , type(append))
        prev_k = k
        while k:
            #print("append" ,append)
            #print("\n")
        ...
        file.write((str({"the_data" :Point.all_data_test ,"privkey":prev_k , "pubkey":""}).replace("'",'"')  ))
        return result
